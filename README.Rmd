...
What exactly is slowing it down? A common recommendation is to use [`profvis`](https://github.com/rstudio/profvis) to find out. 
...
`proffer` uses [`pprof`](https://github.com/google/pprof) to create friendly, fast visualizations.
...
When we navigate to <http://localhost:64610> and select the flame graph, `[<-.data.frame()` jumps out at us immediately. Apparently, the line `x[i, ] <- x[i, ] + 1` is the bottleneck.
...
Indeed, a look at "Top" shows we spend most of the runtime on `x[i, ] <- x[i, ] + 1`. The other operations are fast by comparison.
